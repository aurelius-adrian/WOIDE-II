...
export const EditAnnotationType = ({annotationType}: { annotationType: AnnotationType }) => {
    ...
    const saveAnnotationType = async () => {
        try {
            ...
            if (idx != -1) {
                setDocumentSetting('annotationTypes', prevAnnotationTypes.with(idx, {
                    ...data,
                    id: tmpId
                } as AnnotationType));
            } else {
                setDocumentSetting('annotationTypes', [...prevAnnotationTypes, {
                    ...data,
                    id: tmpId
                }]);
            }
            enqueueSnackbar({message: "Saving Annotation Successful.", variant: "success", autoHideDuration: 2000});
        } catch (error) {
            console.error(error);
            enqueueSnackbar({message: "Saving Annotation Type Failed.", variant: "error", autoHideDuration: 5000});
        }
    }
    ...
    const deleteAnnotationType = async () => {
        try {
            const prevAnnotationTypes = ((await getDocumentSetting('annotationTypes')) ?? []) as AnnotationType[];
            setDocumentSetting('annotationTypes', prevAnnotationTypes.filter((e) => e.id != annotationType.id));
            enqueueSnackbar({message: "Deleting Annotation Successful.", variant: "success", autoHideDuration: 2000});
        } catch (e) {
            enqueueSnackbar({message: "Deleting Annotation Type Failed.", variant: "error", autoHideDuration: 5000});
        }
    }
    ...
    return <div className={"flex flex-col gap-2 items-start"}>
        ...
        <div className={"flex flex-row space-x-2"}>
            ...
            <Button onClick={deleteAnnotationType}>
