...
export const EditAnnotationType = ({ annotationType, setAnnotationType }: { annotationType: AnnotationType, setAnnotationType: (annotationType: AnnotationType | null) => void }) => {
    const formApi = useRef<AnnotationFormApi>(null);
    const handleSave = async () => {
        try {
            const formData = await formApi.current?.submit(); 
            if (formData && formData.name) { 
                await setDocumentSetting(`annotationType_${formData.name}`, formData);
