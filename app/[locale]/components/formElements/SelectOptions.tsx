...
const SelectOption = ({ description }: SelectOptionsProps) => {
    ...
    return (
        <>
            <div className={"mb-4"}>{data && (data as unknown as SelectOptionsData[]).map(getItem)}</div>
            <Button onClick={() => appendFormElementField(defaultEntryData)}>Append Option</Button>
        </>
    );
};
