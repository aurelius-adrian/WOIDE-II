...
export const getAnnotationsInSelection: () => Promise<
  Annotation[]
> = async () => {
...
export const updateAnnotation = async (
  ...
  await Word.run(async (context) => {
    ...
    const toUpdate = contentControls.items.filter(
      (cc) => cc.tag === `${idSalt}_s${AnnotationToUpdateID}`
    );
...
export const _getAnnotationRange = (
  start: Word.Range,
  end: Word.Range
): Word.Range => {
